        ...
        <template #trigger>
            <a class="d-flex align-items-center link-body-emphasis text-decoration-none p-0 dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false">
                <template v-if="account">
                    <img v-if="photo" :src="photo" alt="" width="32" height="32" class="rounded-circle me-2">
                    <strong>{{ name }}</strong>
                </template>
        ...
        <template #default>
            ...
            <ul class="dropdown-menu dropdown-menu-end text-small shadow">
                <template v-if="account">
                    ...
                    <li><a class="dropdown-item" data-test-id="SignOut" @click="systemLogout">Sign Out</a></li>
                </template>
                <template v-else>
